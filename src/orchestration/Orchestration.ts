...
export const defaultCookieAttributes = (): CookieAttributes => {
  return {
    unique: false,
    domain: `${window.location.hostname}.chrome`,
    path: "/",
    sameSite: "Strict",
    secure: true,
  };
...
export class Orchestration {
  ...
  private initDispatch(region: string) {
    ...
    if (this.config.identityPoolId && this.config.guestRoleArn) {
      dispatch.setAwsCredentials(
        new Authentication(this.config).ChainAnonymousCredentialsProvider
      );
  ...
  private initPluginManager(applicationId: string, applicationVersion: string) {
  ...
  private constructBuiltinPlugins(): InternalPlugin[] {
    ...
    this.config.telemetries.forEach((type) => {
      if (typeof type === "string" && functor[type.toLowerCase()]) {
        ...
        plugins = [
          ...plugins,
          ...functor[(type[0] as string).toLowerCase()](type[1] as object),
        ];
      }
    });
